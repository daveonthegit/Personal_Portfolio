...
`prefers-reduced-motion` skips the cinematic and all typing/tick effects — content is
simply present. Every App is keyboard-operable (dock tabbable, Escape closes the focused
window). Assistive tech reads the semantic substrate, never the chrome. Sound is on by
default but begins only at the visitor's first gesture (browser policy), with a
persistent mute.
