...
interface NetworkNode {
  ...
  type: 'server' | 'device' | 'router' | 'security';
  delay: number;
...
interface DataPacket {
  ...
  endY: number;
  speed: number;
  type: 'data' | 'security' | 'surveillance';
  element: HTMLElement;
}
...
class XiaoOSNetworkSurveillance {
  ...
  private generateNodes(): void {
    ...
    const nodeConfigs = [
      { x: 8, y: 15, size: 'large' as const, type: 'server' as const },
      { x: 20, y: 12, size: 'small' as const, type: 'device' as const },
      { x: 35, y: 8, size: 'medium' as const, type: 'router' as const },
      { x: 50, y: 5, size: 'large' as const, type: 'server' as const },
      { x: 70, y: 10, size: 'medium' as const, type: 'security' as const },
      { x: 85, y: 15, size: 'small' as const, type: 'device' as const },
      { x: 92, y: 25, size: 'medium' as const, type: 'router' as const },
      { x: 15, y: 35, size: 'small' as const, type: 'device' as const },
      { x: 30, y: 40, size: 'medium' as const, type: 'security' as const },
      { x: 55, y: 35, size: 'large' as const, type: 'server' as const },
      { x: 75, y: 45, size: 'small' as const, type: 'device' as const },
      { x: 90, y: 50, size: 'medium' as const, type: 'router' as const },
      { x: 10, y: 65, size: 'small' as const, type: 'device' as const },
      { x: 25, y: 70, size: 'medium' as const, type: 'security' as const },
      { x: 45, y: 75, size: 'large' as const, type: 'server' as const },
      { x: 65, y: 80, size: 'small' as const, type: 'device' as const },
      { x: 80, y: 85, size: 'medium' as const, type: 'router' as const },
      { x: 95, y: 90, size: 'small' as const, type: 'device' as const },
    ];
  ...
  private generateNodeLabel(type: string, index: number): string {
    ...
    return `${prefixes[type as keyof typeof prefixes] || 'NODE'}-${String(index + 1).padStart(3, '0')}`;
  }

  private generateConnections(): void {
    const connectionsContainer = this.container.querySelector('.network-connections-container') as HTMLElement;
  ...
  private createConnection(start: NetworkNode, end: NetworkNode, container: HTMLElement): void {
  ...
  private createSurveillanceData(x: number, y: number, value: string, unit: string, label: string): void {
  ...
  private getPacketIcon(type: string): string {
    const icons = {
      'data': '●',
      'security': '🔒',
      'surveillance': '👁'
  ...
  private animateDataPacket(packet: DataPacket): void {
  ...
  private animateConnections(): void {
    const connections = this.container.querySelectorAll('.surveillance-connection');
    connections.forEach(connection => {
      const flowIndicator = connection.querySelector('.data-flow-indicator') as HTMLElement;
      if (flowIndicator) {
...
export function initNetworkVisualization(): void {
  ...
  if (mainContent && !mainContent.querySelector('.xiaoos-network-surveillance')) {
    const visualization = new XiaoOSNetworkSurveillance(mainContent as HTMLElement);
